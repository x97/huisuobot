...
FIELD_ALIASES = {
    "会所名称": ["会所名称"],
    "技师号码": ["技师号码", "老师艺名", "老师花名", "佳人名称", "老师名字", "老师名称"],
    "会所位置": ["会所位置", "联系方式", "佳人牌号", "老师电报"],
    "会所价格": ["会所价格", "所在位置", "所在地区", "上课地点", "老师位置", "老师课室",
                 "上课地址", "深圳地区", "地理位置"],
    "验证留名": ["验证留名", "工兵留名", "出击用户", "学生留名", "出击留名"],
    "验证时间": ["验证时间", "探访时间", "出击时间"],

    "颜值评价": ["颜值评价", "颜值身材", "颜值分数", "身材分数"],
    "身材评价": ["身材评价", "人照几成"],
...
def parse_report(text: str) -> Dict[str, str] | None:
    """
    把抓取到的报告信息转化成模板信息。
